    ...
    <div className={`flex flex-col ${floating ? 'w-[380px] sm:w-[420px] h-[600px] rounded-2xl shadow-2xl' : 'w-full h-[70vh] rounded-xl border'} bg-white overflow-hidden`}>
      ...
      <div className="bg-primary text-white px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[11px] text-white/80 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-300 inline-block" /> Online • Powered by Gemini</div>
        </div>
        {floating && (
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-muted/30">
        {items.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] ${m.role === 'user' ? 'bg-primary text-white' : 'bg-white text-foreground border'} rounded-2xl px-3.5 py-2.5 text-sm whitespace-pre-wrap shadow-sm`}>
              {m.text}
              {m.providers?.length > 0 && (
                <div className="mt-3 space-y-2">
                  {m.providers.slice(0, 4).map(p => (
                    <Link key={p.id} href={p.url} className="flex items-center gap-3 bg-muted/50 hover:bg-accent/10 rounded-lg p-2 border transition-colors">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-[10px] text-muted-foreground">{p.category} • {p.area}, {p.city}</div>
                        <div className="text-[10px] flex items-center gap-1 mt-0.5 text-foreground"><Star className="w-2.5 h-2.5 fill-[#F5A623] text-[#F5A623]" />{p.rating}</div>
                      </div>
                    </Link>
                  ))}
                </div>
        ...
        {busy && (
          <div className="flex justify-start">
            <div className="bg-white border rounded-2xl px-3.5 py-2.5 text-sm text-muted-foreground flex items-center gap-2">
        ...
        {items.length === 1 && (
          <div className="pt-2">
            <div className="text-[11px] text-muted-foreground mb-2">Try asking:</div>
            <div className="flex flex-wrap gap-2">
              {SUGGESTIONS.map(s => (
                <button key={s} onClick={() => send(s)} className="text-xs px-3 py-1.5 rounded-full bg-white border hover:bg-accent/10 hover:text-accent text-muted-foreground">{s}</button>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="p-3 bg-white border-t">
        <div className="flex gap-2">
          ...
          <Button onClick={() => send()} disabled={busy || !text.trim()} className="bg-accent hover:bg-accent/90 text-accent-foreground"><Send className="w-4 h-4" /></Button>
        </div>
        <div className="text-[10px] text-muted-foreground mt-1.5 text-center">AI can make mistakes. Verify important info with the provider directly.</div>
      </div>
    </div>
  );
    ...
    <>
      {open ? (
        ...
        <button
          ...
          className="fixed bottom-6 right-6 z-[60] h-16 pl-4 pr-5 rounded-full bg-accent text-accent-foreground shadow-2xl hover:scale-105 transition-transform flex items-center gap-2.5 group"
        >
