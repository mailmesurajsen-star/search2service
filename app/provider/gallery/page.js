        ...
        <div>
          ...
          {items.length === 0 ? (
            <Card><CardContent className="p-12 text-center text-muted-foreground">
              <ImageIcon className="w-10 h-10 mx-auto mb-2 text-border" />
              No uploads yet.
            </CardContent></Card>
          ) : (
            <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {items.map(m => (
                <Card key={m.id} className="overflow-hidden">
                  <div className="h-36 bg-muted relative group">
                    {m.mimeType?.startsWith('image/') ? (
                      <img src={m.url} alt={m.originalName} className="w-full h-full object-cover" />
                    ) : m.mimeType?.startsWith('video/') ? (
                      <div className="w-full h-full bg-foreground grid place-items-center text-white"><Film className="w-10 h-10" /></div>
                    ) : (
                  ...
                  <CardContent className="p-3">
                    ...
                    <div className="text-[10px] text-muted-foreground">{(m.size/1024).toFixed(0)} KB • {m.context}</div>
                    <div className="flex gap-1 mt-2">
                      <button onClick={() => copyUrl(m.url)} className="flex-1 h-6 rounded bg-accent/10 text-accent text-[10px] font-medium flex items-center justify-center gap-1 hover:bg-accent/20"><Copy className="w-3 h-3" />Copy</button>
                      <a href={m.url} target="_blank" rel="noreferrer" className="flex-1 h-6 rounded bg-primary/10 text-primary text-[10px] font-medium flex items-center justify-center gap-1 hover:bg-primary/15"><ExternalLink className="w-3 h-3" />Open</a>
                    </div>
                  </CardContent>
                </Card>
              ))}
