...
const STATUS_STYLE = {
  ...
  confirmed: { color: 'bg-accent/10 text-accent', icon: CheckCircle2 },
      ...
      <div className="container mx-auto px-4 py-6 max-w-5xl">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          {['all', 'pending', 'confirmed', 'completed', 'cancelled'].map(k => (
            <button key={k} onClick={() => setFilter(k)} className={`p-4 rounded-xl border-2 text-left transition-all ${filter === k ? 'border-accent bg-white shadow' : 'border-transparent bg-white hover:border-border'}`}>
              <div className="text-xs uppercase text-muted-foreground font-medium">{k}</div>
        ...
        {filtered.length === 0 ? (
          <Card><CardContent className="p-12 text-center">
            <ClipboardList className="w-12 h-12 mx-auto text-border mb-3" />
            <div className="font-semibold">No bookings yet</div>
            <div className="text-muted-foreground text-sm mt-1">Complete your business profile so customers can book you.</div>
            <Link href="/provider/business"><Button className="mt-4 bg-accent hover:bg-accent/90 text-accent-foreground">Complete Setup</Button></Link>
          </CardContent></Card>
        ) : (
              ...
              return (
                <Card key={b.id}><CardContent className="p-4">
                  <div className="flex flex-wrap gap-4 items-start">
                    <div className="w-12 h-12 rounded-full bg-primary grid place-items-center text-white font-bold">{b.customerName[0]}</div>
                    <div className="flex-1 min-w-0">
                      ...
                      {b.note && <div className="text-sm text-foreground mt-2 bg-muted/50 rounded p-2">{b.note}</div>}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      ...
                      {b.status === 'confirmed' && <Button size="sm" className="bg-accent hover:bg-accent/90 text-accent-foreground" onClick={() => setStatus(b.id, 'completed')}>Mark Completed</Button>}
