/**
 * India Master Locations Dataset
 * Complete list of all 28 States + 8 Union Territories and their major cities, districts, and localities.
 */
...
/**
 * Get all available States and Union Territories sorted alphabetically
 */
export function getAllStates() {
  return [...INDIA_STATES].sort();
}

/**
 * Get all unique cities across India or filtered by state
 */
export function getCitiesByState(stateName) {
  if (!stateName || stateName === 'all') {
    const set = new Set();
    INDIA_LOCATIONS.forEach(l => { if (l.city) set.add(l.city); });
    return Array.from(set).sort();
  }
  const filtered = INDIA_LOCATIONS.filter(l => l.state.toLowerCase() === stateName.toLowerCase());
  const set = new Set();
  filtered.forEach(l => { if (l.city) set.add(l.city); });
  return Array.from(set).sort();
}
...
export function getDistrictsByState(stateName) {
  ...
  const filtered = INDIA_LOCATIONS.filter(l => l.state.toLowerCase() === stateName.toLowerCase());
  const set = new Set();
  filtered.forEach(l => { if (l.district) set.add(l.district); });
  return Array.from(set).sort();
}

/**
 * Get areas for a city and state
 */
export function getAreasByCity(stateName, cityName) {
  const match = INDIA_LOCATIONS.find(l => {
    const matchState = !stateName || l.state.toLowerCase() === stateName.toLowerCase();
    const matchCity = l.city.toLowerCase() === cityName?.toLowerCase();
    return matchState && matchCity;
  });
  return match?.areas ? [...match.areas].sort() : [];
}

/**
 * Quick search across all states and cities
 */
export function searchLocations(query) {
  ...
  for (const loc of INDIA_LOCATIONS) {
    if (
      ...
      loc.areas.some(a => a.toLowerCase().includes(q))
    ) {
      results.push(loc);
    }
  }
